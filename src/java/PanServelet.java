 ...
 * @author fikar
 */
public class PanServelet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        response.setContentType("text/html;charset=UTF-8");
            ...
            if ("Continue Shopping".equals(request.getParameter("continue")))
             {   this.getServletContext().getRequestDispatcher("/produits.jsp").forward(request, response) ;}
             
            
          
            //Pour supprimer les produits du panier
               if ("Clear Cart".equals(request.getParameter("clearcart")))
             {  
                ...
                String idprod;
                idprod=  request.getParameter("remove");
        
             int indexprod = Integer.parseInt(idprod);
              produit p = pan.getListProduit().get(indexprod - 1) ;
